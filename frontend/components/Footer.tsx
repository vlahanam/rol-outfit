...
export function Footer() {
  ...
  useEffect(() => {
    fetch("/api/v1/settings/social-links")
      .then((res) => res.json())
              ...
              {links?.instagram_url && (
                <li className="flex items-center gap-2">
                  ...
                  <a
                    ...
                    className="hover:text-white transition-colors"
                  >
                    {links.instagram_url.replace(/^https?:\/\/(www\.)?instagram\.com\//, "@").replace(/\/$/, "")}
                  </a>
                </li>
              )}
              {links?.line_url && (
                <li className="flex items-center gap-2">
                  <span className="text-green-500"><LineIcon /></span>
                  ...
                  <a
                    ...
                    className="hover:text-white transition-colors"
                  >
                    {links.line_url.replace(/^https?:\/\/(www\.)?line\.me\/(ti\/p\/|R\/ti\/p\/)?/, "").replace(/\/$/, "")}
                  </a>
                </li>
              )}
              {links?.email && (
